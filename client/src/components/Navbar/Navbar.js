...
export default function Navbar() {
    ...
    return (
        ...
        >
            ...
            <div
                className="bg-blue-300 p-2 rounded-lg font-semibold cursor-pointer relative"
                onClick={() => setCategoryMenuOpen(!categoryMenuOpen)}
            >
                Danh mục sản phẩm
                <FontAwesomeIcon className="ml-4" icon="chevron-down" />
                {categoryMenuOpen && (
                    <div className="absolute top bg-white rounded-lg shadow-lg w-48 p-2">
                        {categories.map((item) => (
